...
def add_record_to_record_list(record_list: T.List[Record], record: Record):
    found = False
    for rec in record_list:
        if (
            rec.map.id == record.map.id or rec.map.name == record.map.name
        ) and record.points > rec.points:
...
def get_player_records(steamid64):
    ...
    if response.status_code == 200:
        ...
        for record in json_data:
            # Invalid map check
            if get_map(record["map_id"]) is None:
                continue
...
def get_total_points(record_list: T.List[Record], difficulty: int) -> int:
    if difficulty == 0:
        return sum(record.points for record in record_list)

    return len([r for r in record_list if r.map.difficulty == difficulty])


def get_num_record_difficulty(record_list: T.List[Record], difficulty: int) -> int:
    if difficulty == 0:
...
def get_average_points(record_list: T.List[Record], difficulty: int) -> float:
...
def get_num_records_at_min_point(record_list: T.List[Record], threshold: int) -> int:
    return len([r for r in record_list if r.points >= threshold])


def get_num_records_at_difficulty_and_min_point(
    record_list: T.List[Record], difficulty: int, threshold: int
) -> int:
    return len(
        [
            r
            for r in record_list
            if (r.points >= threshold and r.map.difficulty == difficulty)
        ]
    )
...
def get_rank(nub_records, pro_records):
    count_t567_p800 = (
        get_num_records_at_difficulty_and_min_point(nub_records, 5, 800)
    ...
    count_t567_p900 = (
        get_num_records_at_difficulty_and_min_point(nub_records, 5, 900)
    ...
    count_t567_pro = (
        get_num_record_difficulty(pro_records, 5)
        + get_num_record_difficulty(pro_records, 6)
        + get_num_record_difficulty(pro_records, 7)
    )
    count_map_t567 = MAP_TIERS["5"] + MAP_TIERS["6"] + MAP_TIERS["7"]

    rank_factor = (
        (get_num_records_at_min_point(pro_records, 1000) / len(MAPS)) ** (1 / 2.4) * 6
        + (get_num_records_at_min_point(nub_records, 1000) / len(MAPS)) ** (1 / 3.5)
        * 1.1
    ...
    pt_factor = (
        (get_average_points(nub_records, 0) / 1000) * 0.07
    ...
    compl_factor = (
        (get_num_record_difficulty(nub_records, 0) / len(MAPS)) * 0.01
